<template>
    <li class="border-2 rounded-lg p-8 relative shadow-sm" :class="`shadow-${move.type}/80 border-${move.type}`">
        <div class="w-4/5">
            <p class="py-2 px-4 w-fit rounded-lg font-semibold text-lg text-white inline" :class="`bg-${move.type}`">{{
                capitalize(move.name) }}
            </p>
            <p v-if="move.level" class="ml-2 font-semibold inline text-lg" :class="`text-${move.type}`">Lvl. {{ move.level }}</p>
            <p class="mt-4">{{ move.description }}</p>
            <div class="flex flex-wrap gap-4 mt-4">
                <template v-for="info in infos" :key="info.name">
                    <p v-if="info.visible" class="bg-gray-200 py-1 px-2 rounded">
